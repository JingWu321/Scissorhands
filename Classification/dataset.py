...
def celeba_dataloaders(
    batch_size=128,
    data_dir="/home/jing/datasets/CelebAMaskHQ/CelebA_HQ_facial_identity_dataset",
    num_workers=2,
    random_to_replace: int = None,
    class_to_replace: int = None,
    num_indexes_to_replace=None,
    ...
    print(
        "Dataset information: CelebA Mask HQ"
    )

    train_set = ImageFolder(os.path.join(data_dir, 'train'), transform=train_transform) # 4263
    test_set = ImageFolder(os.path.join(data_dir, 'test'), transform=test_transform) # 1215

    train_set.targets = np.array(train_set.targets)
    train_set.targets = np.array(train_set.targets)
    rng = np.random.RandomState(seed)

    # all: 307 identities, unl: 10% from all, rem: 90% from all
    unl_identities = np.random.choice(np.unique(train_set.targets), int(0.1 * len(np.unique(train_set.targets))), replace=False)
    # unl_identities = np.array([172, 131, 7, 84, 164, 295, 241, 300, 141, 112, 204, 282, 291, 65, 179, 94, 173, 268, 215, 13, 3, 66, 30, 262, 29, 89, 180, 182, 181, 191])
    rem_identities = np.setdiff1d(np.unique(train_set.targets), unl_identities)

    unl_idx = np.where(np.isin(train_set.targets, unl_identities))[0]
    rem_idx = np.where(np.isin(train_set.targets, rem_identities))[0]
    forget_set = Subset(train_set, unl_idx)
    remain_set = Subset(train_set, rem_idx)

    test_rem_idx = np.where(np.isin(test_set.targets, rem_identities))[0]
    test_remain_set = Subset(test_set, test_rem_idx)
