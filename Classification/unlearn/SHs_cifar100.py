...
def snip(net, dataloader, sparsity, prune_num, device, arch='resnet18'):
    ...
    with torch.no_grad():
        ...
        if arch == 'resnet18':
            ...
            for (name, param), (m_param) in zip(net.named_parameters(), mask_.parameters()):
                if ('patch_embed' not in name) and ('pos_embed' not in name) and ('cls_token' not in name) and(('blocks.10' in name) or ('blocks_final' in name)):
                    mask = torch.empty(param.data.shape, device=device)
                    if 'head' in name:
                        re_init_param = torch.nn.init.zeros_(mask)
                    elif ('norm' in name) or ('bn' in name):
                        if ('weight' in name):
                            re_init_param = torch.nn.init.ones_(mask)
                        elif ('bias' in name):
                            re_init_param = torch.nn.init.zeros_(mask)
                    else:
                        if ('weight' in name):
                            re_init_param = timm.models.layers.weight_init.trunc_normal_(mask, std=0.02)
                        elif ('bias' in name):
                            re_init_param = torch.nn.init.zeros_(mask)
                    param.data = param.data * m_param.data + re_init_param.data * (1 - m_param.data)
...
def SHs(data_loaders, model, criterion, optimizer, epoch, args, mask=None):
    ...
    if args.arch == 'vit':
        condition = lambda n: ('patch_embed' not in n) and ('pos_embed' not in n) and ('cls_token' not in n) and(('blocks.10' in n) or ('blocks_final' in n) )
    elif args.arch == 'resnet18':
        condition = lambda n: (('layer3' in n) or ('layer4' in n) or ('fc' in n))

    # prune via snip
    if epoch == 0:
        model = snip(model, forget_loader, args.sparsity, args.prune_num, device=device, arch=args.arch)
